  ...
  return (
    <div className="space-y-1.5 min-w-40">
      <div className="flex items-center gap-2">
        <span className="text-[10px] font-bold text-ink-3 w-16 shrink-0">Available</span>
        <div className="flex-1 h-2 rounded-full bg-surface-2 overflow-hidden">
          <div className="h-full rounded-full bg-primary" style={{ width: `${(available / scale) * 100}%` }} />
        </div>
        <span className="text-[10px] font-bold text-ink-2 w-8 text-right">{available}h</span>
      </div>
      <div className="flex items-center gap-2">
        <span className="text-[10px] font-bold text-ink-3 w-16 shrink-0">Required</span>
        <div className="flex-1 h-2 rounded-full bg-surface-2 overflow-hidden">
          <div className={`h-full rounded-full ${reqMax > available ? 'bg-red' : 'bg-green'}`} style={{ width: `${(reqMax / scale) * 100}%` }} />
        </div>
        <span className="text-[10px] font-bold text-ink-2 w-8 text-right">{reqMax}h</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="anim-fade-up">
      <div className="flex flex-wrap items-end justify-between gap-3 mb-5">
        <div>
          <h1 className="text-xl font-extrabold text-ink">Approvals</h1>
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-start gap-3 flex-1 min-w-60">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-ink">{a.applicantName}</p>
                    <p className="text-[11px] text-ink-3">{a.applicantRole} · {a.applicantDepartment}</p>
      ...
      {waiting.length > 0 && (
        <div className="mt-6">
          <h2 className="text-sm font-bold text-ink mb-3">Blocked on registration ({waiting.length})</h2>
          <div className="space-y-2">
            {waiting.map((a) => (
              <Card key={a.id} className="p-4 flex items-center gap-3">
                <Avatar initials={a.applicantInitials} size="sm" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-ink truncate">{a.applicantName} → {a.postTitle}</p>
                  <p className="text-[11px] text-violet font-semibold">Waiting for the admin to register this person's manager.</p>
                </div>
      ...
      <div className="mt-6">
        <button onClick={() => setShowHistory((v) => !v)} className="text-xs font-bold text-ink-2 hover:text-ink">
      ...
      >
        <div className="space-y-4">
          ...
          {decide?.decision === 'approved' && decide.app.aiRecommendation === 'Not Recommended' && (
            <p className="text-xs font-semibold text-amber bg-amber-soft rounded-xl px-3 py-2.5">
